...
# Last updated: 27th August, 2021 - 18.30 (BST)

## Empty list object
weekly_data = list()
## List all the .csv file names (and sort them in case they are not ordered by number)
file_names = sort(list.files('~/Documents/github/bikes/data/', pattern='.csv'))
...
for(week in (n_weeks-5):(n_weeks-2)){
  weekly_data_train[[week]] = read.table(paste('~/Documents/github/bikes/data/',file_names[week],sep=''), 
...
for(week in (n_weeks-1):n_weeks){
  weekly_data_test[[week]] = read.table(paste('~/Documents/github/bikes/data/',file_names[week],sep=''), 
                                        sep=',', header=FALSE, col.names=c('start_id','end_id','start_time','duration'))
}
...
## Import stations
stations = read.table('~/Documents/github/bikes/data/santander_locations.csv', sep=',', header=TRUE)

## Load some utility functions
source('~/Documents/github/bikes/hawkes_utility.R')
...
boxplot(ks_scores_all ~ ks_factors, col=c('lightgreen','lightblue','pink','gray'), 
        xlab='Model', ylab='KS scores', main='Boxplots of KS scores - Training set', ylim=c(0,0.5))
boxplot(ks_scores_all_test ~ ks_factors_test, col=c('lightgreen','lightblue','pink','gray'), 
        xlab='Model', ylab='KS scores', main='Boxplots of KS scores - Test set', ylim=c(0,0.5))

## Number of observations vs. KS-score 
## ** EXTREMELY IMPORTANT ** Shows that the full model is much better than the others
...
plot(len_train, unlist(ks_pois_train), ylim=c(0,0.5), pch=19, cex=.25, 
     xlab='Number of observations', ylab='KS scores', main='Poisson')
abline(lm(unlist(ks_pois_train) ~ len_train)$coef)
plot(len_train, unlist(ks_mod1_train), ylim=c(0,0.5), pch=19, cex=.25, 
     xlab='Number of observations', ylab='KS scores',col='Firebrick3', main='Model 1')
...
plot(len_train, unlist(ks_mod2_train), ylim=c(0,0.5), pch=19, cex=.25, 
     xlab='Number of observations', ylab='KS scores',col='Blue3', main='Model 2')
